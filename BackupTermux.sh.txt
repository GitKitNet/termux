...
function CREATE_DIRECTORY() {
	...
	BACKUPtar_REPO="$BACKUP_REPODIR/$BACKUP_TARFILE"
	
	if [[ ! -e $BACKUP_REPODIR ]]; then
		echo "NOT exists $BACKUP_REPODIR . Now create dir"; mkdir -p $BACKUP_REPODIR;
	elif [[ ! -d $BACKUP_REPODIR ]]; then
		echo "Is exists - $BACKUP_REPODIR, but is not a dir!!!" 1>&2
	fi;
...
function ResticRepo() {
	...
	if [[ ! -e $RESTIC_REPOSITORY ]];
		then
			...
			mkdir $RESTIC_REPOSITORY | VARIABLE_RESTIC
...
function RESTORE_TERMUX_RESTIC() {
	echo '## ==================================== ##'
...
function BACKUP_TERMUX_TAR() {
	...
	## Backing up files:
	tar -zcvf $BACKUPtar_REPO home usr;
...
read n
case $n in
  1) echo -an "Backing up Termux - RESTIC" | sleep 3; BACKUP_TERMUX_RESTIC ;;
  2) echo -an "Restoring Termux  - RESTIC" | sleep 3; RESTORE_TERMUX_RESTIC ;;
  3) echo -an "Backing up Termux    - TAR" | sleep 3; BACKUP_TERMUX_TAR ;;
  4) echo -an "Restoring Termux     - TAR" | sleep 3; RESTORE_TERMUX_TAR ;;
  *) echo -an "Select is ERROR.\nExit Now" | sleep 3; exit ;;
esac
